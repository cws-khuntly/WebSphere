...
(
                            ...
                            if [[ ! -s "${installation_script}" ]]; then
                                ...
                                if [[ -z "${ret_code}" ]] || (( ret_code != 0 )); then
                                    ...
                                    if [[ -z "${ret_code}" ]] || (( ret_code != 0 )); then
                                        ...
                                        ## done
                                        [[ "${LOGGING_LOADED}" == "${_TRUE}" ]] && writeLogEntry "INFO" "${cname}" "${function_name}" "${LINENO}" "dotfiles successfully installed to host ${target_host}";
                                    fi
                                fi
                            fi
                        fi
    ...
    if [[ -n "${ENABLE_DEBUG}" ]] && [[ "${ENABLE_DEBUG}" == "${_TRUE}" ]] && [[ "${LOGGING_LOADED}" == "${_TRUE}" ]]; then
        writeLogEntry "DEBUG" "${cname}" "${function_name}" "${LINENO}" "cleanup_list -> ${cleanup_list}";
        writeLogEntry "DEBUG" "${cname}" "${function_name}" "${LINENO}" "EXEC: cleanupFiles ${CLEANUP_LOCATION_REMOTE} ${cleanup_list} ${target_host} ${target_port} ${target_user}";
    fi
